...
// legacyApply applies the regex-based dialect transform to a SQL fragment.
func legacyApply(input string, from, to dialect.Dialect) (string, error) {
	var out string
	switch {
	...
	case from == dialect.SQLite && to == dialect.MariaDB:
		...
		out = doubleQuotedIdentToBackticks(out)
...
func sqliteToMariaDB() []rule {
	return []rule{
		{mustRE(`\bINSERT\s+OR\s+IGNORE\s+INTO\b`), `INSERT IGNORE INTO`},
		{mustRE(`\bINSERT\s+OR\s+REPLACE\s+INTO\b`), `REPLACE INTO`},
